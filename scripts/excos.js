import { excosList } from "../data/executives.js";


const productContainer = document.getElementById('product-container')

loadexcosList(excosList, productContainer)

export function loadexcosList(productData,div) {
  let productHTML = '';
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      <div class="pt-6">
        <div class="mb-4 flex items-center justify-between gap-4">
          <span class="me-2 rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-[#1e40af] dark:bg-primary-900 dark:text-primary-300"> Up to ${product.position}% off </span>
        ...
        <div class="mt-2 flex items-center gap-2">
          ...
          <p class="text-sm font-medium text-gray-900 dark:text-white">${product.id.group}</p>
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400">(${product.id.number})</p>
        </div>
        ...
        <div class="mt-4 flex items-center justify-between gap-4">
          <p class="text-2xl font-extrabold leading-tight text-gray-900 dark:text-white">$${product.position}</p>

          <button type="button" class="inline-flex items-center rounded-lg bg-[#1d4ed8] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#1e40af] focus:outline-none focus:ring-4  focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-[#1d4ed8] dark:focus:ring-[#1e40af]">
  ...
  div.insertAdjacentHTML('beforeend', productHTML);

  /*div.querySelectorAll('.product-row').forEach(row => {
    row.addEventListener('click', function() {
      // Your function here
      alert('Row clicked!');
      renderproductID();
      // You can also access row data if needed
    });
  });
